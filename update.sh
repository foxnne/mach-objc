...
for pair in \
    'MACHWindow_aarch64-macos.s aarch64-macos.12.0' \
    'MACHWindow_x86_64-macos.s x86_64-macos.12.0'
do
    dst=${pair%% *}
    target=${pair#* }

    zig cc -c MACHWindow.m \
        -target "$target" \
        -S -Os -fomit-frame-pointer -fobjc-arc -fno-objc-exceptions \
        -o "$dst" \
        -iframework ./xcode-frameworks/Frameworks \
...
mv MACHWindow_aarch64-macos.s MACHWindow_arm64_apple_macos12.s
mv MACHWindow_x86_64-macos.s MACHWindow_x86_64_apple_macos12.s
...
for pair in \
    'MACHApplication_aarch64-macos.s aarch64-macos.12.0' \
    'MACHApplication_x86_64-macos.s x86_64-macos.12.0'
do
    dst=${pair%% *}
    target=${pair#* }

    zig cc -c MACHApplication.m \
        -target "$target" \
        -S -Os -fomit-frame-pointer -fobjc-arc -fno-objc-exceptions \
        -o "$dst" \
        -iframework ./xcode-frameworks/Frameworks \
...
mv MACHApplication_aarch64-macos.s MACHApplication_arm64_apple_macos12.s
mv MACHApplication_x86_64-macos.s MACHApplication_x86_64_apple_macos12.s
